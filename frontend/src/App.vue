    ...
    <section class="workspace">
      ...
      <section class="tool">
        ...
        <section v-if="active === 'wrong'" class="stack">
          <form class="form" @submit.prevent="createWrong">
...
async function createWrong() {
  ...
  message.value = (await request(path, { method, body: JSON.stringify(wrongForm) })).message
...
async function deleteWrong(id) {
  ...
  message.value = (await request(`/api/wrong-analysis/${id}`, { method: 'DELETE' })).message
  await loadActive()
}
