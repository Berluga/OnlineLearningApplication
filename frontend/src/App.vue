    ...
    <section class="workspace">
      ...
      <section class="tool">
        ...
        <section class="feature-card">
          <div class="query-panel" aria-label="查询条件">
            <div>
              <h4>查询功能</h4>
              <p>先选择查询对象，再输入对应字段。这里只展示当前查询对象的结果。</p>
            </div>
            <form class="search-form" @submit.prevent>
              <label>查询对象
                <select v-model="queryTarget">
                  <option v-for="target in queryTargets" :key="target.key" :value="target.key">{{ target.label }}</option>
                </select>
              </label>
              <label>查询内容<input v-model="queryKeyword" :placeholder="currentQueryTarget.placeholder" /></label>
              <button type="button" @click="showAllQueryRows">显示全部</button>
            </form>
          </div>
          <div class="section-heading">
            <h4>{{ currentQueryTarget.label }}查询结果</h4>
            <p>可按：{{ currentQueryTarget.fields }} 查询。</p>
          </div>
          <DataTable :columns="currentQueryColumns" :rows="currentQueryRows" :empty="currentQueryEmpty" />
        </section>

        <section v-if="active === 'question'" class="stack">
          <section class="feature-card">
            <h4>新增 / 修改 / 删除题目</h4>
            <form class="form" @submit.prevent="saveQuestion">
              ...
              <label>题型
                <select v-model="questionForm.type">
                  <option v-for="type in questionTypes" :key="type.name" :value="type.name">{{ type.name }}</option>
                </select>
              </label>
            ...
            <form v-if="questionForm.type === '选择题'" class="form compact-options" @submit.prevent="saveQuestion">
              <label>A选项<input v-model="questionForm.optionA" placeholder="A选项内容" /></label>
              <label>B选项<input v-model="questionForm.optionB" placeholder="B选项内容" /></label>
              <label>C选项<input v-model="questionForm.optionC" placeholder="C选项内容" /></label>
              <label>D选项<input v-model="questionForm.optionD" placeholder="D选项内容" /></label>
              <label>正确选项<select v-model="questionForm.correctOption"><option>A</option><option>B</option><option>C</option><option>D</option></select></label>
            </form>
            <p v-if="areaMessage('question')" class="inline-message">{{ areaMessage('question') }}</p>
            <DataTable :columns="questionColumns" :rows="rows" empty="暂无题目">
              <template #actions="{ row }">
                <button type="button" @click="fillImageForm(row, 'question')">传题图</button>
                <button type="button" @click="fillImageForm(row, 'answer')">传答图</button>
          ...
          <section class="feature-card">
            <h4>维护题型</h4>
            <form class="form two" @submit.prevent="saveType">
              <label>题型名称<input v-model="typeForm.name" required placeholder="例如：判断题" /></label>
              <label>题型说明<input v-model="typeForm.description" placeholder="例如：后续可扩展的新题型" /></label>
              <button type="submit">{{ typeForm.oldName ? '保存题型' : '新增题型' }}</button>
            </form>
            <p v-if="areaMessage('type')" class="inline-message">{{ areaMessage('type') }}</p>
            <DataTable :columns="typeColumns" :rows="questionTypes" empty="暂无题型">
              <template #actions="{ row }">
                <button type="button" @click="editType(row)">修改</button>
                <button type="button" class="danger" @click="deleteType(row.name)">删除</button>
              </template>
            </DataTable>
          </section>
          ...
          <section class="feature-card">
            <h4>上传题目图片</h4>
            <form class="form" @submit.prevent="uploadQuestionImage">
              <label>题目ID<input v-model="imageForm.questionId" type="number" required placeholder="要上传图片的题目ID" /></label>
              <label>图片用途<select v-model="imageForm.imageRole"><option value="question">题目贴图</option><option value="answer">回答图片</option></select></label>
              <label>图片地址<input v-model="imageForm.imageUrl" required placeholder="例如：http://localhost:8080/images/q1.png" /></label>
              <button type="submit">上传题目图片</button>
            </form>
            <p v-if="areaMessage('image')" class="inline-message">{{ areaMessage('image') }}</p>
            <DataTable :columns="questionImageColumns" :rows="questionImageRows" empty="暂无题目图片" />
          </section>
        </section>
        ...
        <section v-if="active === 'meta'" class="stack">
          <section class="feature-card">
            ...
            <p v-if="areaMessage('chapter')" class="inline-message">{{ areaMessage('chapter') }}</p>
          ...
          <section class="feature-card">
            ...
            <p v-if="areaMessage('point')" class="inline-message">{{ areaMessage('point') }}</p>
          ...
          <section class="feature-card">
            ...
            <p v-if="areaMessage('answer')" class="inline-message">{{ areaMessage('answer') }}</p>
          ...
          <section class="feature-card">
            ...
            <p v-if="areaMessage('mistake')" class="inline-message">{{ areaMessage('mistake') }}</p>
        ...
        <section v-if="active === 'paper'" class="stack">
          ...
          <p v-if="areaMessage('paper')" class="inline-message">{{ areaMessage('paper') }}</p>
          <h4>试卷列表</h4>
        ...
        <section v-if="active === 'student'" class="stack">
          ...
          <p v-if="areaMessage('student')" class="inline-message">{{ areaMessage('student') }}</p>
          <h4>学生列表</h4>
          ...
          <p v-if="areaMessage('department')" class="inline-message">{{ areaMessage('department') }}</p>
          ...
          <p v-if="areaMessage('class')" class="inline-message">{{ areaMessage('class') }}</p>
          ...
          <p v-if="areaMessage('course')" class="inline-message">{{ areaMessage('course') }}</p>
          ...
          <p v-if="areaMessage('selection')" class="inline-message">{{ areaMessage('selection') }}</p>
          ...
          <form class="form" @submit.prevent="createScore">
            <label>录入成绩学生
              <select v-model="scoreForm.studentId" required>
                <option value="">请选择学生</option>
                <option v-for="student in studentData.students || []" :key="student.student_id" :value="student.student_id">{{ student.name }}</option>
              </select>
            </label>
            <label>考试/科目名称<input v-model="scoreForm.paperTitle" required placeholder="例如：Java第四次测验" /></label>
            <label>成绩<input v-model="scoreForm.score" type="number" min="0" max="100" required placeholder="例如：88" /></label>
            <button type="submit">录入成绩并刷新波动图</button>
          </form>
          <p v-if="analysisMessage" class="analysis-note">{{ analysisMessage }}</p>
          <div class="trend-chart" v-if="trendPoints.length > 1">
            <svg viewBox="0 0 640 220" role="img" aria-label="成绩波动折线图">
              <line x1="44" y1="20" x2="44" y2="180" />
              <line x1="44" y1="180" x2="620" y2="180" />
              <polyline :points="trendLinePoints" />
              <g v-for="point in trendPoints" :key="point.result_id">
                <circle :cx="point.x" :cy="point.y" r="5" />
                <text :x="point.x" :y="point.y - 10">{{ point.score }}</text>
                <text class="trend-label" :x="point.x" y="202">{{ point.label }}</text>
              </g>
            </svg>
          </div>
...
const message = ref('')
const lastActionArea = ref('')
const queryKeyword = ref('')
const queryTarget = ref('question')
const dbStatus = ref('数据库检查中')

const questionTypes = ref([{ name: '选择题', description: '系统内置题型' }, { name: '填空题', description: '系统内置题型' }, { name: '主观题', description: '系统内置题型' }])
const questionForm = reactive({ id: '', stem: '', type: '选择题', difficulty: '中等', optionA: '', optionB: '', optionC: '', optionD: '', correctOption: 'A' })
const typeForm = reactive({ oldName: '', name: '', description: '' })
const imageForm = reactive({ questionId: '', imageRole: 'question', imageUrl: '' })
...
const scoreForm = reactive({ studentId: '', paperTitle: 'Java第四次测验', score: 88 })
...
const subsystems = [
  ...
  { key: 'meta', name: '题目性质管理子系统', title: '答案、知识点、章节、易错点维护', scope: '维护答案、知识点、章节、难易度、出错率和易错点，为组卷、考试和错题分析提供基础数据。', useCases: ['查询题目性质', '维护标准答案', '维护知识点', '设置难易度', '维护章节', '统计出错率', '维护易错点'] },
  { key: 'paper', name: '自动组卷子系统', title: '自动组卷', scope: '根据总分、题型数量、难易度和知识点覆盖要求从题库中自动抽题并生成试卷。', useCases: ['查询试卷', '设置组卷条件', '自动抽取题目', '校验知识点覆盖', '计算题目分值', '生成试卷', '预览试卷'] },
  { key: 'student', name: '学生管理子系统', title: '学生增删改查', scope: '学生的院系班级分配、学生以班级为单位选课、学生管理和学生情况分析。', useCases: ['查询学生信息', '学生的院系班级分配', '学生以班级为单位选课', '学生管理（增删查改）', '学生情况分析（历次考试成绩波动图、错误知识点统计报告）'] },
  { key: 'wrong', name: '错题分析子系统', title: '错题记录与分析', scope: '对考试结果进行错题归纳，统计班级和个人在章节、知识点、难易度维度上的错误情况，并给出加强建议。', useCases: ['查询错题记录', '导入考试结果', '分析班级错题', '分析个人错题', '统计知识点错误率', '生成学习建议', '查看正确答案'] },
]

const current = computed(() => subsystems.find((item) => item.key === active.value))
const trendPoints = computed(() => {
  const items = studentAnalysis.value.scores || []
  ...
  return items.map((item, index) => {
    ...
    return {
      ...item,
      score,
      x: items.length === 1 ? left + width / 2 : left + (width * index) / (items.length - 1),
      y: top + height - (Math.max(0, Math.min(score, 100)) / 100) * height,
      label: item.paper_title || `考试${index + 1}`,
    }
  })
})
const trendLinePoints = computed(() => trendPoints.value.map((point) => `${point.x},${point.y}`).join(' '))

const questionColumns = [{ key: 'question_id', label: 'ID' }, { key: 'type', label: '题型' }, { key: 'stem', label: '题干' }, { key: 'difficulty', label: '难易度' }, { key: 'option_text', label: '选择题选项' }, { key: 'answer_text', label: '答案' }, { key: 'analysis_text', label: '解析' }, { key: 'question_image_text', label: '题目图片' }, { key: 'answer_image_text', label: '回答图片' }]
const questionImageColumns = [{ key: 'question_id', label: '题目ID' }, { key: 'type', label: '题型' }, { key: 'stem', label: '题干' }, { key: 'image_role_text', label: '图片用途' }, { key: 'image_url', label: '图片地址' }]
const typeColumns = [{ key: 'name', label: '题型' }, { key: 'description', label: '说明' }]
...
const queryTargets = computed(() => ({
  question: [
    { key: 'question', label: '题目', fields: '题目ID、题干、题型、难易度、选项内容、答案、解析、图片地址', placeholder: '例如：1 / 选择题 / 中等 / 极限 / 答案', columns: questionColumns, rows: () => rows.value },
    { key: 'type', label: '题型', fields: '题型名称、题型说明', placeholder: '例如：选择题 / 主观题 / 系统内置', columns: typeColumns, rows: () => questionTypes.value },
  ],
  meta: [
    { key: 'chapter', label: '章节', fields: '章节ID、章节名称、排序', placeholder: '例如：函数与极限 / 1', columns: chapterColumns, rows: () => meta.value.chapters || [] },
    { key: 'point', label: '知识点', fields: '知识点ID、章节ID、知识点名称、描述', placeholder: '例如：极限定义 / 核心要点', columns: pointColumns, rows: () => meta.value.knowledgePoints || [] },
    { key: 'answer', label: '答案', fields: '答案ID、题目ID、答案内容、解析', placeholder: '例如：参考答案 / 解题思路 / 题目ID', columns: answerColumns, rows: () => meta.value.answers || [] },
    { key: 'mistake', label: '易错点', fields: '易错点ID、题目ID、知识点ID、易错描述', placeholder: '例如：常见错误 / 知识点ID', columns: mistakeColumns, rows: () => meta.value.mistakePoints || [] },
  ],
  paper: [
    { key: 'paper', label: '试卷', fields: '试卷ID、试卷名称、总分、难易度、创建人ID', placeholder: '例如：第一章测验 / 100 / 困难', columns: paperColumns, rows: () => paper.value.papers || [] },
    { key: 'paperQuestion', label: '试卷题目', fields: '试卷ID、题目ID、分值、序号', placeholder: '例如：题目ID / 10 / 序号', columns: paperQuestionColumns, rows: () => paper.value.paperQuestions || [] },
  ],
  student: [
    { key: 'student', label: '学生', fields: '学生ID、姓名、院系、班级、年级', placeholder: '例如：张三 / 软件工程2301 / 计算机学院', columns: studentColumns, rows: () => studentData.value.students || [] },
    { key: 'department', label: '院系', fields: '院系ID、院系名称', placeholder: '例如：计算机学院 / 院系ID', columns: departmentColumns, rows: () => studentData.value.departments || [] },
    { key: 'class', label: '班级', fields: '班级ID、班级名称、年级、院系', placeholder: '例如：软件工程2301 / 2023', columns: classColumns, rows: () => studentData.value.classes || [] },
    { key: 'course', label: '课程', fields: '课程ID、课程名称、学科、说明', placeholder: '例如：高等数学 / 数学', columns: courseColumns, rows: () => studentData.value.courses || [] },
    { key: 'selection', label: '选课记录', fields: '选课ID、学生、班级、课程、选课时间', placeholder: '例如：张三 / 高等数学 / 选课时间', columns: selectionColumns, rows: () => studentData.value.selections || [] },
    { key: 'weakPoint', label: '薄弱知识点', fields: '知识点ID、知识点名称、错误次数、涉及学生', placeholder: '例如：极限定义 / 张三 / 错误次数', columns: weakPointColumns, rows: () => studentAnalysis.value.weakPoints || [] },
  ],
  wrong: [
    { key: 'wrong', label: '错题记录', fields: '错题ID、考试结果ID、题目ID、知识点ID、错误原因', placeholder: '例如：知识点掌握不牢 / 题目ID / 错题ID', columns: wrongColumns, rows: () => rows.value },
  ],
}[active.value] || []))

const currentQueryTarget = computed(() => queryTargets.value.find((target) => target.key === queryTarget.value) || queryTargets.value[0] || { label: '记录', fields: '关键字段', placeholder: '请输入查询内容', columns: [], rows: () => [] })
const currentQueryColumns = computed(() => currentQueryTarget.value.columns)
const currentQueryRows = computed(() => filterRows(currentQueryTarget.value.rows()))
const currentQueryEmpty = computed(() => queryKeyword.value.trim()
  ? '没有匹配的查询结果'
  : `暂无${currentQueryTarget.value.label}，录入数据后会默认显示全部结果`
)
const questionImageRows = computed(() => rows.value.flatMap((question) => (question.images || []).map((image) => ({
  question_id: question.question_id,
  type: question.type,
  stem: question.stem,
  image_role_text: image.image_role === 'answer' ? '回答图片' : '题目贴图',
...
async function loadActive() {
  try {
    if (active.value === 'question') {
      const data = await request('/api/question')
      questionTypes.value = data.types || questionTypes.value
      rows.value = (data.items || []).map(normalizeQuestionRow)
    }
    ...
    showAreaMessage(active.value, `加载数据失败：${error.message}`)
  }
}
...
async function saveQuestion() {
  try {
    ...
    const result = await request(path, { method, body: JSON.stringify(buildQuestionPayload()) })
    showAreaMessage('question', result.message)
    resetQuestionForm()
    await loadActive()
  } catch (error) {
    showAreaMessage('question', `保存题目失败：${error.message}`)
  }
}
...
function filterRows(rows) {
  ...
  return list.filter((row) => Object.values(row || {}).some((value) => String(value ?? '').toLowerCase().includes(keyword)))
}

function editQuestion(row) {
  const options = Object.fromEntries((row.options || []).map((option) => [option.label, option]))
  Object.assign(questionForm, {
    id: row.question_id,
    stem: row.stem,
    type: row.type,
    difficulty: row.difficulty,
    optionA: options.A?.content || '',
    optionB: options.B?.content || '',
    optionC: options.C?.content || '',
    optionD: options.D?.content || '',
    correctOption: (row.options || []).find((option) => option.is_correct)?.label || 'A',
  })
}

async function deleteQuestion(id) {
  showAreaMessage('question', (await request(`/api/question/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}

async function saveType() {
  const path = typeForm.oldName ? '/api/question/type' : '/api/question/type'
  const method = typeForm.oldName ? 'PUT' : 'POST'
  showAreaMessage('type', (await request(path, { method, body: JSON.stringify(typeForm) })).message)
  Object.assign(typeForm, { oldName: '', name: '', description: '' })
  await loadActive()
}

function editType(row) {
  Object.assign(typeForm, { oldName: row.name, name: row.name, description: row.description || '' })
}

async function deleteType(name) {
  if (!confirm(`确定删除题型“${name}”吗？已被题目使用的题型不能删除。`)) return
  showAreaMessage('type', (await request(`/api/question/type/${encodeURIComponent(name)}`, { method: 'DELETE' })).message)
  await loadActive()
}

function fillImageForm(row, imageRole) {
  Object.assign(imageForm, { questionId: row.question_id, imageRole, imageUrl: '' })
}

async function uploadQuestionImage() {
  showAreaMessage('image', (await request(`/api/question/${imageForm.questionId}/image`, { method: 'POST', body: JSON.stringify(imageForm) })).message)
  Object.assign(imageForm, { questionId: '', imageRole: 'question', imageUrl: '' })
  await loadActive()
}

function showAreaMessage(area, text) {
  lastActionArea.value = area
  message.value = text
}

function areaMessage(area) {
  return lastActionArea.value === area ? message.value : ''
}
...
function buildQuestionPayload() {
  ...
  if (questionForm.type === '选择题') {
    payload.options = ['A', 'B', 'C', 'D']
      .map((label) => ({
        label,
        content: questionForm[`option${label}`],
        isCorrect: questionForm.correctOption === label,
      }))
      .filter((option) => option.content && option.content.trim())
  }
  return payload
}

function resetQuestionForm() {
  Object.assign(questionForm, { id: '', stem: '', type: '选择题', difficulty: '中等', optionA: '', optionB: '', optionC: '', optionD: '', correctOption: 'A' })
}

function normalizeQuestionRow(row) {
  const images = row.images || []
  const questionImages = images.filter((image) => image.image_role !== 'answer')
  const answerImages = images.filter((image) => image.image_role === 'answer')
  return {
    ...row,
    option_text: (row.options || []).map((option) => `${option.label}.${option.content}${option.is_correct ? '（正确）' : ''}`).join('；') || '-',
    answer_text: (row.answers || []).map((answer) => answer.answer_content).join('；') || '-',
    analysis_text: (row.answers || []).map((answer) => answer.analysis).filter(Boolean).join('；') || '-',
    question_image_text: questionImages.map((image) => image.image_url).join('；') || '-',
    answer_image_text: answerImages.map((image) => image.image_url).join('；') || '-',
  }
}
...
async function saveStudent() {
  ...
  showAreaMessage('student', (await request(path, { method, body: JSON.stringify(studentForm) })).message)
...
async function deleteStudent(id) {
  showAreaMessage('student', (await request(`/api/student/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}
...
async function saveDepartment() {
  ...
  showAreaMessage('department', (await request(path, { method, body: JSON.stringify(departmentForm) })).message)
...
async function deleteDepartment(id) {
  ...
  showAreaMessage('department', (await request(`/api/student/department/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}
...
async function saveClass() {
  ...
  showAreaMessage('class', (await request(path, { method, body: JSON.stringify(classForm) })).message)
...
async function deleteClass(id) {
  ...
  showAreaMessage('class', (await request(`/api/student/class/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}
...
async function saveCourse() {
  ...
  showAreaMessage('course', (await request(path, { method, body: JSON.stringify(courseForm) })).message)
...
async function deleteCourse(id) {
  ...
  showAreaMessage('course', (await request(`/api/student/course/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}

async function selectCourseByClass() {
  showAreaMessage('selection', (await request('/api/student/class-course-selection', { method: 'POST', body: JSON.stringify(selectionForm) })).message)
  Object.assign(selectionForm, { classId: '', courseId: '' })
  await loadActive()
}

async function deleteCourseSelection(id) {
  showAreaMessage('selection', (await request(`/api/student/course-selection/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}
...
async function loadStudentAnalysis() {
  ...
  try {
    ...
    const scores = await request(`/api/student/score-trend${suffix}`)
    const report = await request(`/api/student/weak-point-report${suffix}`)
    studentAnalysis.value = {
      scores: scores.items || [],
      weakPoints: report.items || [],
      summary: {
        scoreCount: scores.count || 0,
        weakPointCount: report.summary?.weakPointCount || 0,
...
async function createScore() {
  showAreaMessage('student', (await request('/api/student/score', { method: 'POST', body: JSON.stringify(scoreForm) })).message)
  Object.assign(scoreForm, { studentId: '', paperTitle: 'Java第四次测验', score: 88 })
  await loadStudentAnalysis()
}

async function createChapter() {
  if (chapterForm.id) {
    showAreaMessage('chapter', (await request(`/api/question-meta/chapter/${chapterForm.id}`, { method: 'PUT', body: JSON.stringify({ name: chapterForm.name, orderNo: chapterForm.orderNo }) })).message)
  } else {
    showAreaMessage('chapter', (await request('/api/question-meta/chapter', { method: 'POST', body: JSON.stringify(chapterForm) })).message)
  }
...
async function deleteChapter(id) {
  ...
  showAreaMessage('chapter', (await request(`/api/question-meta/chapter/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}

async function createKnowledgePoint() {
  if (pointForm.id) {
    showAreaMessage('point', (await request(`/api/question-meta/knowledge-point/${pointForm.id}`, { method: 'PUT', body: JSON.stringify({ chapterId: pointForm.chapterId, name: pointForm.name, description: pointForm.description }) })).message)
  } else {
    showAreaMessage('point', (await request('/api/question-meta/knowledge-point', { method: 'POST', body: JSON.stringify(pointForm) })).message)
  }
...
async function deleteKnowledgePoint(id) {
  ...
  showAreaMessage('point', (await request(`/api/question-meta/knowledge-point/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}

async function createAnswer() {
  if (answerForm.id) {
    showAreaMessage('answer', (await request(`/api/question-meta/answer/${answerForm.id}`, { method: 'PUT', body: JSON.stringify({ questionId: answerForm.questionId, answerContent: answerForm.answerContent, analysis: answerForm.analysis }) })).message)
  } else {
    showAreaMessage('answer', (await request('/api/question-meta/answer', { method: 'POST', body: JSON.stringify(answerForm) })).message)
  }
...
async function deleteAnswer(id) {
  ...
  showAreaMessage('answer', (await request(`/api/question-meta/answer/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}

async function createMistakePoint() {
  if (mistakeForm.id) {
    showAreaMessage('mistake', (await request(`/api/question-meta/mistake-point/${mistakeForm.id}`, { method: 'PUT', body: JSON.stringify({ questionId: mistakeForm.questionId, pointId: mistakeForm.pointId, description: mistakeForm.description }) })).message)
  } else {
    showAreaMessage('mistake', (await request('/api/question-meta/mistake-point', { method: 'POST', body: JSON.stringify(mistakeForm) })).message)
  }
...
async function deleteMistakePoint(id) {
  ...
  showAreaMessage('mistake', (await request(`/api/question-meta/mistake-point/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}

async function generatePaper() {
  showAreaMessage('paper', (await request('/api/auto-paper/generate', { method: 'POST', body: JSON.stringify(paperForm) })).message)
  await loadActive()
}

async function deletePaper(id) {
  if (!confirm('确定删除这张试卷吗？相关试卷题目、考试、成绩和错题记录也会一起删除。')) return
  showAreaMessage('paper', (await request(`/api/auto-paper/${id}`, { method: 'DELETE' })).message)
  await loadActive()
}
...
watch(active, async () => {
  queryKeyword.value = ''
  queryTarget.value = defaultQueryTarget(active.value)
  await loadActive()
})
onMounted(async () => {
  queryTarget.value = defaultQueryTarget(active.value)
...
function defaultQueryTarget(key) {
  return { question: 'question', meta: 'chapter', paper: 'paper', student: 'student', wrong: 'wrong' }[key] || 'question'
}

function showAllQueryRows() {
  queryKeyword.value = ''
}
</script>
...
.feature-card { padding: 14px; border: 1px solid #e2e8f0; border-radius: 8px; background: #fff; }
.form { display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 12px; align-items: end; margin: 18px 0; }
...
.tool button, .form button { justify-self: start; white-space: nowrap; }
.message { margin: 12px 0; padding: 10px 12px; border: 1px solid #add8bb; border-radius: 6px; background: #effaf2; color: #17653a; }
.inline-message { margin: 8px 0 12px; padding: 9px 12px; border: 1px solid #add8bb; border-radius: 6px; background: #effaf2; color: #17653a; }
.query-panel { display: grid; grid-template-columns: minmax(260px, 0.9fr) minmax(420px, 1.1fr); gap: 16px; align-items: end; margin: 14px 0 18px; padding: 14px; border: 1px solid #d8dee9; border-radius: 8px; background: #f8fafc; }
.query-panel p { margin: 8px 0 0; color: #64748b; line-height: 1.6; }
.search-form { display: grid; grid-template-columns: 160px minmax(220px, 1fr) auto; gap: 10px; align-items: end; margin: 0; }
.query-result { margin: 0 0 22px; padding-bottom: 16px; border-bottom: 1px solid #e2e8f0; }
.section-heading { display: flex; align-items: end; justify-content: space-between; gap: 12px; margin-bottom: 8px; }
.section-heading p { margin: 0; color: #64748b; line-height: 1.5; }
.analysis-note { margin: -4px 0 4px; color: #31516f; }
.trend-chart { margin: 14px 0; border: 1px solid #d7e0eb; border-radius: 8px; padding: 12px; overflow-x: auto; }
.trend-chart svg { width: 100%; min-width: 620px; height: 220px; }
.trend-chart line { stroke: #cbd5e1; stroke-width: 2; }
.trend-chart polyline { fill: none; stroke: #225ea8; stroke-width: 4; stroke-linecap: round; stroke-linejoin: round; }
.trend-chart circle { fill: #225ea8; }
.trend-chart text { fill: #0f172a; font-size: 13px; text-anchor: middle; font-weight: 700; }
.trend-chart .trend-label { fill: #475569; font-size: 11px; font-weight: 500; }
...
table { width: 100%; border-collapse: collapse; margin-top: 14px; background: #fff; display: block; overflow-x: auto; }
th, td { border-bottom: 1px solid #e2e8f0; padding: 10px; text-align: left; vertical-align: top; }
th { background: #f8fafc; color: #334155; }
.danger { color: #b42318; }
@media (max-width: 900px) { .workspace, .form, .form.two, .form.compact, .query-panel, .search-form, .bar-row { grid-template-columns: 1fr; } .topbar { align-items: flex-start; flex-direction: column; } }
</style>
